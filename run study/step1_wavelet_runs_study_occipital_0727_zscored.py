...
for subject in tqdm(subj_lst):
    ...
    roi = fmri_dataset(subject+'/occipital.nii') #load mask
    #roi.shape
    ...
    orig_roi = roi.a.mapper.reverse(roi.samples)
    #orig_roi.shape
    new_data = orig_roi * orig_data
    
    # z-score for each run
    
    for run in range(16):#16 is the total number of runs
        # tr_index = np.arange(512)
        # run_mask = np.logical_and(run*32 <= tr_index, tr_index < (run+1)*32)
        run_mask = runs == run
        new_data_run = new_data[runs==run,:,:,:]
        new_data_run_z = stats.zscore(new_data_run, axis=0)
        # voxels that are not in ROI has values of 0 constantly, this will give
        # nan when being z-scored, now we remove these nan by replacing them with
        # zeros, i.e. find all nan and replace with 0
        new_data_run_z[np.isnan(new_data_run_z)] = 0
        new_data[run_mask,:,:,:] = new_data_run_z
    ...
    np.save("runs_study_occipital_zscored"+subject,ALL_Level_TR.data)
